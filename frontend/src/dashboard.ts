// Ported 1:1 from templates/dashboard.html's inline <script>: polls
// /api/data, renders KPIs/trend/donut/breakdowns as HTML strings (kept as
...
const fmtInt = (n: number | null | undefined): string => Math.round(n ?? 0).toLocaleString("ru-RU");
const SERIES = ["var(--series-1)", "var(--series-2)", "var(--series-3)", "var(--series-4)", "var(--series-5)", "var(--series-6)"];
const fmtDisp = (n: number): string => (window.DISPLAY_CURRENCY === "UZS" ? fmtInt(n) : fmt(n));

function toDisplay(uzs: number | undefined, usd: number | undefined): number {
  uzs = uzs || 0;
  usd = usd || 0;
...
function setStatus(
  ...
  const pill = document.getElementById("statusPill") as HTMLElement;
  ...
  const note = document.getElementById("progressNote") as HTMLElement;
  note.textContent = progress && progress.total ? `${progress.done} / ${progress.total} ${window.T["dash.progress_suffix"]}` : "";
}

function renderBreakdown(byMap: Record<string, ChannelStat> | undefined): string {
  const list = Object.entries(byMap || {}).map(([name, s]) => {
    const cur = s.currency;
    const nativeVal = (s.revenue && s.revenue[cur]) || 0;
    return [name, s, toDisplay(cur === "UZS" ? nativeVal : 0, cur === "USD" ? nativeVal : 0)] as const;
  });
  ...
  const legend = list
    .map(([name], i) => `<div class="legend-item"><span class="legend-dot" style="background:${SERIES[i % SERIES.length]}"></span>${name}</div>`)
    .join("");
  const rows = list
    .map(([name, s, val], i) => {
...
function renderDonut(byMap: Record<string, ChannelStat> | undefined): string {
  const list = Object.entries(byMap || {}).map(([name, s]) => {
    const cur = s.currency;
    const nativeVal = (s.revenue && s.revenue[cur]) || 0;
    return [name, s, toDisplay(cur === "UZS" ? nativeVal : 0, cur === "USD" ? nativeVal : 0)] as const;
  ...
  const MAX_SLICES = 5;
  let shown: (readonly [string, { count: number }, number])[] = list.slice(0, MAX_SLICES);
...
function renderStayStats(stats: StayStats | undefined): string {
  ...
  return (
    `<div class="kpi-row">` +
    items.map(([label, val]) => `<div class="kpi"><div class="label">${label}</div><div class="value mono">${val}</div></div>`).join("") +
    `</div>`
  );
}

function renderTrendChart(
  byMonth: Record<string, Partial<{ UZS: number; USD: number }>> | undefined,
  countMonth: Record<string, Partial<{ UZS: number; USD: number }>> | undefined,
): string {
  ...
  const yBottomPct = (v: number) => 5 + (v / max) * 90;
  const yTopPct = (v: number) => 100 - yBottomPct(v);
  ...
  const labels = months
    .map((m, i) => (i % labelEvery === 0 || i === n - 1 ? `<div class="trend-label" style="left:${xPct(i)}%">${m.slice(5, 7)}/${m.slice(2, 4)}</div>` : ""))
    .join("");
...
function renderNetResultHero(agg: AggregateData, manual: ManualSummary): string {
...
function render(payload: ApiDataPayload): void {
  ...
  const otherCurAmount =
    window.DISPLAY_CURRENCY === "USD"
      ? (revenue.UZS || 0) + (manual.income.UZS || 0) + (manual.expense.UZS || 0)
      : (revenue.USD || 0) + (manual.income.USD || 0) + (manual.expense.USD || 0);
...
const PERIOD_QS = new URLSearchParams(window.location.search).toString();

async function poll(): Promise<void> {
  try {
    const res = await fetch("/api/data" + (PERIOD_QS ? "?" + PERIOD_QS : ""));
    const payload = (await res.json()) as ApiDataPayload;
    render(payload);
  } catch (e) {
    console.error(e);
  }
